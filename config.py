...
# Commercial backends:
# 'Campfire' - see https://campfirenow.com/ (follow instructions from
#              https://github.com/errbotio/err-backend-campfire)
# 'Hipchat'  - see https://www.hipchat.com/
# 'Slack'    - see https://slack.com/
# 'Gitter'   - see https://gitter.im/ (follow instructions from
#              https://github.com/errbotio/err-backend-gitter)

# Open protocols:
# 'TOX'      - see https://tox.im/ (follow instructions from
#              https://github.com/errbotio/err-backend-tox)
...
BACKEND = os.environ.get('ERRBOT_BACKEND', 'Slack')
...
STORAGE = os.environ.get('ERRBOT_STORAGE', 'Shelf')

BOT_EXTRA_STORAGE_PLUGINS_DIR = os.environ.get(
    'ERRBOT_BOT_EXTRA_STORAGE_PLUGINS_DIR'
)

# The location where all of Err's data should be stored. Make sure to set
# this to a directory that is writable by the user running the bot.
BOT_DATA_DIR = os.environ.get('ERRBOT_BOT_DATA_DIR', '/data')

### Repos and plugins config.

# Set this to change from where errbot gets its installable plugin list.
# By default it gets the index from errbot.io which is a file generated by
# tools/plugin-gen.py.
BOT_PLUGIN_INDEXES = os.environ.get(
    'ERRBOT_BOT_PLUGIN_INDEXES',
...
BOT_EXTRA_PLUGIN_DIR = os.environ.get('ERRBOT_BOT_EXTRA_PLUGIN_DIR', '/plugins')

# If you use an external backend as a plugin,
# this is where you tell Errbot where to find it.
BOT_EXTRA_BACKEND_DIR = os.environ.get('ERRBOT_BOT_EXTRA_BACKEND_DIR')

# If you want only a subset of the core plugins that are bundled with errbot,
# you can specify them here.
_CORE_PLUGINS = os.environ.get('ERRBOT_CORE_PLUGINS')
CORE_PLUGINS = _CORE_PLUGINS.split(',') if _CORE_PLUGINS else None
# For example CORE_PLUGINS = ('ACLs', 'Backup', 'Help') you get those
# names from the .plug files Name entry.
# For absolutely no plug: CORE_PLUGINS = ()

# Defines an order in which the plugins are getting their callbacks. Useful
# if you want to have plugins do pre- or post-processing on messages.
# The 'None' tuple entry represents all the plugins that aren't to be
# expicitely ordered. For example, if you want 'A' to run first, then
# everything else but 'B', then 'B', you would use ('A', None, 'B').
_CALLBACK_ORDER = os.environ.get('ERRBOT_PLUGINS_CALLBACK_ORDER')
PLUGINS_CALLBACK_ORDER = _CALLBACK_ORDER.split(',') if _CALLBACK_ORDER else (None, )
...
AUTOINSTALL_DEPS = bool(int(
    os.environ.get('ERRBOT_AUTOINSTALL_DEPS', 0)
))

# The location of the log file. If you set this to None, then logging will
# happen to console only.
BOT_LOG_FILE = os.environ.get('ERRBOT_BOT_LOG_FILE', '/log/err.log')
...
BOT_LOG_LEVEL = LOG_LEVELS[os.environ.get('ERRBOT_BOT_LOG_LEVEL', 'INFO')]

# Enable logging to sentry (find out more about sentry at www.getsentry.com).
# This is optional and disabled by default.

BOT_LOG_SENTRY = bool(int(
    os.environ.get('ERRBOT_BOT_LOG_SENTRY', 0)
))

SENTRY_DSN = os.environ.get('ERRBOT_SENTRY_DSN')

SENTRY_LOGLEVEL = LOG_LEVELS[os.environ.get('ERRBOT_SENTRY_LOGLEVEL', 'ERROR')]
...
# single command.
BOT_ASYNC = bool(int(
    os.environ.get('ERRBOT_BOT_ASYNC', 1)
))

# Size of the thread pool for the asynchronous mode.
BOT_ASYNC_POOLSIZE = int(os.environ.get('ERRBOT_BOT_ASYNC_POOLSIZE', 10))
...
BOT_IDENTITY = json.loads(
    os.environ.get('ERRBOT_BOT_IDENTITY', '{}')
)
...
BOT_ADMINS = os.environ.get('ERRBOT_BOT_ADMINS', '').split(',')
...
_CHATROOMS = os.environ.get('ERRBOT_CHATROOM_PRESENCE', '')
CHATROOM_PRESENCE = _CHATROOMS.split(',') if _CHATROOMS else ()
...
CHATROOM_FN = os.environ.get('ERRBOT_CHATROOM_FN', 'Errbot')
...
#
BOT_PREFIX = os.environ.get('ERRBOT_BOT_PREFIX', '!')
#
...
BOT_PREFIX_OPTIONAL_ON_CHAT = bool(int(
    os.environ.get('ERRBOT_BOT_PREFIX_OPTIONAL_ON_CHAT', 0)
))
...
# the prefix above.
BOT_ALT_PREFIXES = os.environ.get(
...
#
BOT_ALT_PREFIX_SEPARATORS = os.environ.get(
...
# 'err' or even 'ERR'.
BOT_ALT_PREFIX_CASEINSENSITIVE = bool(int(
    os.environ.get('ERRBOT_BOT_ALT_PREFIX_CASEINSENSITIVE', 1)
))
...
# Example:
#
ACCESS_CONTROLS_DEFAULT = json.loads(
    os.environ.get('ERRBOT_ACCESS_CONTROLS_DEFAULT', '{}')
)

ACCESS_CONTROLS = json.loads(
    os.environ.get('ERRBOT_ACCESS_CONTROLS', '{}')
)


# Uncomment and set this to True to hide the restricted commands from
# the help output.
HIDE_RESTRICTED_COMMANDS = bool(int(
    os.environ.get('ERRBOT_HIDE_RESTRICTED_COMMANDS', 0)
))
...
HIDE_RESTRICTED_ACCESS = bool(int(
    os.environ.get('ERRBOT_HIDE_RESTRICTED_ACCESS', 0)
))
...
DIVERT_TO_PRIVATE = os.environ.get('ERRBOT_DIVERT_TO_PRIVATE', '').split(',')
...
CHATROOM_RELAY = json.loads(
    os.environ.get('ERRBOT_CHATROOM_RELAY', '{}')
)
...
REVERSE_CHATROOM_RELAY = json.loads(
    os.environ.get('ERRBOT_REVERSE_CHATROOM_RELAY', '{}')
)
...
MESSAGE_SIZE_LIMIT = os.environ.get('ERRBOT_MESSAGE_SIZE_LIMIT', 10000)
...
XMPP_CA_CERT_FILE = os.environ.get(
    'ERRBOT_XMPP_CA_CERT_FILE',
...
XMPP_FEATURE_MECHANISMS = json.loads(
    os.environ.get('ERRBOT_XMPP_FEATURE_MECHANISMS', '{}')
)
...
XMPP_KEEPALIVE_INTERVAL = int(
    os.environ.get('ERRBOT_XMPP_KEEPALIVE_INTERVAL', 300)
)
...
XMPP_XHTML_IM = bool(int(
    os.environ.get('ERRBOT_XMPP_XHTML_IM', 0)
))
...
IRC_CHANNEL_RATE = int(os.environ.get('ERRBOT_IRC_CHANNEL_RATE', 1))
IRC_PRIVATE_RATE = int(os.environ.get('ERRBOT_IRC_PRIVATE_RATE', 1))
IRC_RECONNECT_ON_KICK = int(os.environ.get('ERRBOT_IRC_RECONNECT_ON_KICK', 5))
IRC_RECONNECT_ON_DISCONNECT = int(
    os.environ.get('ERRBOT_IRC_RECONNECT_ON_DISCONNECT', 5)
)
...
IRC_ACL_PATTERN = os.environ.get(
    'ERRBOT_IRC_ACL_PATTERN', '{nick}!{user}@{host}'
)

# Allow messages sent in a chatroom to be directed at requester.
GROUPCHAT_NICK_PREFIXED = bool(int(
    os.environ.get('ERRBOT_GROUPCHAT_NICK_PREFIXED', 0)
))

# Disable table borders, making output more compact (supported only on IRC,
# Slack and Telegram currently).
COMPACT_OUTPUT = bool(int(os.environ.get('ERRBOT_COMPACT_OUTPUT', 1)))

# Disables the logging output in Text mode and only outputs Ansi.
TEXT_DEMO_MODE = bool(int(os.environ.get('ERRBOT_TEXT_DEMO_MODE', 0)))

# Prevent ErrBot from saying anything if the command is unrecognized.
SUPPRESS_CMD_NOT_FOUND = bool(int(
    os.environ.get('ERRBOT_SUPPRESS_CMD_NOT_FOUND', 0)
))
